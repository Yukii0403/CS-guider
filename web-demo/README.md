...
本目录的 `worker.js` 里内置了两段提示词：

- `SYSTEM_PROMPTS.chat` —— 领域定位与路线规划
- `SYSTEM_PROMPTS.analyze` —— 从项目代码反推能力

**这两段提示词必须与专家智能体版本里的 SKILL.md 保持同一份文本。** 两个交付形态（公网页面 / 专家智能体）共享一套 AI 逻辑，只换外壳，**避免日后改了一边忘了另一边**。

建议做法：把提示词抽成一个独立文件（如 `prompts.md`）放在仓库根目录，两边都从它复制。
